...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non EUB unit input in lnReqAmount, we generate the URI with EUB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(EurobitUnits::format(EurobitUnits::EUB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
