...
const int EUROBIT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString EUROBIT_IPC_PREFIX("eurobit:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(EUROBIT_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(EUROBIT_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(EUROBIT_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click eurobit: links)
...
{
    // clicking on eurobit: URLs creates FileOpen events on the Mac:
